...
# Model
M_LSTM_VO = LSTM_VO(par.img_h, par.img_w, par.batch_norm)
print(M_LSTM_VO)
use_cuda = torch.cuda.is_available()
if use_cuda:
    print('CUDA used.')
    M_LSTM_VO = M_LSTM_VO.cuda()
...
if par.pretrained_flownet and not par.resume:
	...
	# Use only conv-layer-part of FlowNet as CNN for LSTM_VO
	model_dict = M_LSTM_VO.state_dict()
	update_dict = {k: v for k, v in pretrained_w['state_dict'].items() if k in model_dict}
	model_dict.update(update_dict)
	M_LSTM_VO.load_state_dict(model_dict)


# Create optimizer
if par.optim['opt'] == 'Adam':
	optimizer = torch.optim.Adam(M_LSTM_VO.parameters(), lr=0.001, betas=(0.9, 0.999))
elif par.optim['opt'] == 'Adagrad':
	optimizer = torch.optim.Adagrad(M_LSTM_VO.parameters(), lr=par.optim['lr'])
elif par.optim['opt'] == 'Cosine':
	optimizer = torch.optim.SGD(M_LSTM_VO.parameters(), lr=par.optim['lr'])
...
# Load trained LSTM_VO model and optimizer
if par.resume:
	M_LSTM_VO.load_state_dict(torch.load(par.load_model_path))
...
for ep in range(par.epochs):
	...
	for _, t_x, t_y in train_dl:
		...
		ls = M_LSTM_VO.step(t_x, t_y, optimizer).data.cpu().numpy()
	...
	for _, v_x, v_y in valid_dl:
		...
		v_ls = M_LSTM_VO.get_loss(v_x, v_y).data.cpu().numpy()
	...
	if loss_mean_valid < min_loss_v and ep % check_interval == 0:
		...
		torch.save(M_LSTM_VO.state_dict(), par.save_model_path+'.valid')
	...
	if loss_mean < min_loss_t and ep % check_interval == 0:
		...
		torch.save(M_LSTM_VO.state_dict(), par.save_model_path+'.train')
