...
if __name__ == '__main__':    
	...
	if use_cuda:
		...
		M_LSTM_VO.load_state_dict(torch.load(load_model_path))
	else:
		M_LSTM_VO.load_state_dict(torch.load(load_model_path, map_location={'cuda:0': 'cpu'}))
	...
	for test_video in videos_to_test:
		...
		for i, batch in enumerate(dataloader):
			...
			if use_cuda:
				x = x.cuda()
				y = y.cuda()
			batch_predict_pose = M_LSTM_VO.forward(x)
